...
def build_node_geometry(year: str | int, root: str | Path) -> dict[str, Any]:
    """Derive stable page and widget geometry from inventories and field maps."""
    ...
    for field_map in load_field_maps(year, root):
        ...
        inventory_payload = _load_inventory(field_map, root)
        for page in inventory_payload.get("pages", []) or []:
            if not isinstance(page, dict):
                continue
            document_id = str(field_map["document_id"])
            page_number = int(page["page"])
            pages_by_key[(document_id, page_number)] = {
    ...
    return {
        "tax_year": int(year),
        "pages": [pages_by_key[key] for key in sorted(pages_by_key)],
        "entries": entries,
    }


def _load_inventory(field_map: dict[str, Any], root: str | Path) -> dict[str, Any]:
    """Load one extracted inventory, including its source page geometry."""
    path = Path(root) / str(field_map["inventory"])
...
def validate_node_geometry(year: str | int, root: str | Path) -> list[str]:
    """Validate schema, reproducibility, and fail-closed page bounds."""
    root_path = Path(root)
    ...
    errors.extend(_validate_page_bounds(committed))
    return errors


def _validate_page_bounds(geometry: dict[str, Any]) -> list[str]:
    """Require captured dimensions for every widget page and keep rects on-page."""
    pages = {
        (str(item["document_id"]), int(item["page"])): item
    ...
    for entry in geometry.get("entries", []) or []:
        if not isinstance(entry, dict):
            continue
        key = (str(entry.get("document_id") or ""), int(entry.get("page", 0)))
        page = pages.get(key)
        if page is None:
            errors.append(
                f"node geometry -> missing captured page dimensions for {key[0]} page {key[1]}"
            )
            continue
        rect = entry.get("rect")
        ...
        if x0 < 0 or y0 < 0 or x1 > float(page["width"]) or y1 > float(page["height"]):
            errors.append(
                f"node geometry -> rect outside page box for {entry.get('document_id')} "
                f"page {entry.get('page')} field {entry.get('field_name')}"
            )
    return errors
