...
class OperationSpec:
    ...
    def role_contracts(self) -> tuple[tuple[str, ...], ...]:
        """Return the accepted required-role sequences for extraction."""
        return (self.roles, *self.role_variants)
...
OPERATION_SPECS: tuple[OperationSpec, ...] = (
    ...
    OperationSpec("SUM", "Add one or more values.", ("addend",), 1, None, "sum", "sum_currency", ("addend",), expandable=True, repeatable_roles=("addend",)),
    ...
    OperationSpec("MIN", "Use the smallest of two or more values.", ("candidate",), 2, None, "min", "min_currency", ("candidate",), expandable=True, repeatable_roles=("candidate",)),
    OperationSpec("MAX", "Use the largest of two or more values.", ("candidate",), 2, None, "max", "max_currency", ("candidate",), expandable=True, repeatable_roles=("candidate",)),
    ...
    OperationSpec("LOOKUP_BRACKET", "Calculate a value from a bracket table.", ("amount", "brackets"), 2, 2, "lookup_bracket", "lookup_bracket_tax", ("amount",), repeatable_roles=("brackets",)),
    ...
    OperationSpec("IF_ELSE", "Choose between two values using a comparison.", ("condition", "threshold", "when_true", "when_false"), 4, 4, "if_else", None, ("condition", "threshold", "when_true", "when_false"), repeatable_roles=("threshold", "when_true", "when_false"), role_variants=(("when_true", "when_false"),)),
    OperationSpec("AND", "Require every predicate to be true.", ("candidate",), 2, None, "and", None, predicate=True, expandable=True, category="predicate", runtime_notes="Predicate expressions are represented inside a condition, not as standalone graph rules.", repeatable_roles=("candidate",)),
    OperationSpec("OR", "Accept when any predicate is true.", ("candidate",), 2, None, "or", None, predicate=True, expandable=True, category="predicate", runtime_notes="Predicate expressions are represented inside a condition, not as standalone graph rules.", repeatable_roles=("candidate",)),
...
def operation_required_roles(operation: str) -> tuple[str, ...]:
    """Return the primary required-role sequence for one operation."""
    spec = operation_spec(operation)
    return spec.roles if spec is not None else ()


def operation_repeatable_roles(operation: str) -> tuple[str, ...]:
    """Return roles that may occur more than once in a micro plan."""
    spec = operation_spec(operation)
    return spec.repeatable_roles if spec is not None else ()


def assign_operation_roles(
    operation: str,
    observed_roles: Sequence[str | None],
) -> tuple[str, ...] | None:
    """Assign and validate operand roles without counting operands as lines.

    ``None`` means that the source item is positional and should receive the
    next role from the registry.  Explicit roles must belong to one accepted
    role sequence.  A role may repeat only when the registry marks it as
    repeatable, which is how printed tables and filing-status variants remain
    representable without inventing operation names.
    """
    spec = operation_spec(operation)
    if spec is None or spec.named_leaf_roles:
        return None

    for contract in spec.role_contracts():
        assigned = _assign_roles_for_contract(contract, spec.repeatable_roles, observed_roles)
        if assigned is not None:
            return assigned
    return None


def _assign_roles_for_contract(
    contract: tuple[str, ...],
    repeatable_roles: tuple[str, ...],
    observed_roles: Sequence[str | None],
) -> tuple[str, ...] | None:
    """Backtrack the small role vocabulary to fill positional omissions."""
    assigned: list[str] = []

    def visit(index: int) -> bool:
        if index == len(observed_roles):
            counts = {role: assigned.count(role) for role in contract}
            return all(
                counts[role] >= 1 and (role in repeatable_roles or counts[role] == 1)
                for role in contract
            )

        explicit = observed_roles[index]
        candidates = [explicit] if explicit is not None else [
            role for role in contract
            if role not in assigned or role in repeatable_roles
        ]
        for candidate in candidates:
            if candidate is None or candidate not in contract:
                continue
            if candidate not in repeatable_roles and candidate in assigned:
                continue
...
__all__ = [
    ...
    "operation_repeatable_roles",
    "operation_required_roles",
    "operation_roles",
    "assign_operation_roles",
    "operation_spec",
