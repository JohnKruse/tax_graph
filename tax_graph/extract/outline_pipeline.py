...
def _record_union_non_computation(
    ...
    *,
    ...
    """Project non-computation union answers into existing coverage telemetry."""
    ...
    cell["has_form_face_citation"] = any(span.relationship == "source" for span in spans if span.span_id in citation_span_ids)
    if kind == "not_derivable":
        _record_review_gap(stats, cell, str(plan.get("reason") or "model could not derive this line"))
        return
    ...
    record: dict[str, Any] = {
        ...
        "label": cell["label"],
        "status": "complete",
        "source_kind": kind if kind in {"form_line", "filer_entry", "information_return"} else "information_return",
        "form": str(plan.get("form") or ""),
        "line": "",
        "box": str(plan.get("box") or ""),
        "quote": quote,
    ...
    if resolved:
        stats["source_cells_resolved"] += 1
        stats["resolved_source_addresses"].append(cell["target_cell_id"])
        return
