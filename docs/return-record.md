...
> "Tax Trace" (§5.3) / `export_audit_file` (§8.2) from *computation provenance* into a
...
5. **Computed outputs + trace summary** — the §5.3 Tax Trace for the final values.
...
is never uploaded, and is the user's to keep. Test/example records use only fake data (§10.5).

## MVP note

Even the capital-gains MVP has a carryforward (the capital-loss carryover). The *computation*
of that carryover is deferred for v0 (req. doc §9.3), but the **Return Record structure should
