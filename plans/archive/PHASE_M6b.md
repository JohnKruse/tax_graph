# PHASE M6b - Repeatable-table execution (row instances)   [COMPLETE]
...
- [DONE] **Step 6 - Widen the M6 oracle harness to N lots.** Scenario model gains multiple lots
  ...
  - Worker note: widened `CapitalGainScenario` with explicit multi-lot rows while preserving the
    single-lot constructor path, rendered Tax Graph table facts and multi-row OTS 8949 CSVs, and
    changed the committed domain profile to 1..15 long-term lots with mixed gain/loss and nonzero
    column (g). Live OTS fuzz `--n 100 --seed 2468` agreed 100/100; the frozen corpus was
    regenerated as seed `20260706` with `live_ots_diff_report` provenance and replayed cleanly.
