# PHASE M12 - Output layer   [COMPLETE]

**Closed 2026-07-10 by the Architect (Claude Sonnet 5).** ROLE DEVIATION NOTE
(M8/M11-close precedent): Codex implemented Steps 1-3 and had Steps 4-5 complete in the
worktree when its session hit a 5-hour usage limit mid-Step-3-commit; the human directed
the Architect to finish the phase directly. Close-out: split the uncommitted worktree
into per-step commits (Steps 1-2 were already committed by Codex; 3/4/5 committed here
matching the plan's step boundaries), then ran the exit criteria for real. That run
caught a genuine bug the offline test suite could not see: `export_filing_bundle` never
resolved the real OTS-shipped template, so every emitted sidecar used the generic
fallback template, whose injected numeric fields (L1a/L2b/L3a/L3b, D14/D19/Collectibles)
were not semicolon-terminated - OTS's parser reads a numeric field as a value list that
only closes at a `;`, so the unterminated fields silently swallowed the next line's
label and the real `taxsolve_US_1040_2025.exe` failed outright on a freshly emitted
sidecar. Fixed both the missing template auto-resolution (config-driven, optional,
same gated posture as every other OTS touchpoint) and the fallback template's
terminators; four M6/M11 offline goldens that had encoded the unparseable text were
regenerated. Re-ran live: OTS agreed exactly with our line 16 (13777.00) on the fixed
sidecar. This is the same class of gap M11's rounding bug was: offline goldens
internally consistent, live oracle catches what they can't see - worth naming since it
means offline-only verification is not sufficient proof for the output layer's
user-facing artifacts.

Full pytest 281 passed / 4 skipped; `pytest -m m12` 22 passed; ASCII OK; `validate`/
base-deps `run`/`frontier` green; parity line 7 = 2000 and 250 confirmed on yaml AND
the existing (pre-M12, still valid) sqlite build. `build 2025` itself could not be
re-run this session - four `tax-graph serve` MCP processes already running in this
environment hold `build/tax_graph_2025.sqlite` open, and Windows will not let the
build step delete-and-replace a file another process has open; killing background MCP
server processes without the human's go-ahead was judged out of scope for this
close-out. The existing sqlite artifact is unaffected in content (M12 added no graph
nodes/rules, only output/field-map layers) and both parity checks pass against it, so
this is recorded as a known, non-content-affecting gap rather than worked around.
VERIFICATION.md + 11 pages regenerated byte-stable (no diff - this phase changed no
modeled math, matching the guardrail that the output layer must not move frontier
coverage). Deferred-review queue: field_map_review entries for all 11 M12-mapped forms
(pending, high priority, human_confirmed: false).
...
- [DONE] **Step 3 [worker-standard] - OTS input sidecar.** Reuse the differential
...
- [DONE] **Step 4 [worker-standard] - Return-scoped output contract + MCP surface.**
...
- [DONE] **Step 5 [worker-standard] - Node-to-page geometry for the workbench.** Derive
...
- [DONE] **Step 6 [worker-light] - Records, frontier, exit run.** Regenerate
