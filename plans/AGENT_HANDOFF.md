...
**BALL: CODEX. M20-S116 is in progress below - bidirectional instruction reconciliation and safe
owner-map consumption.**
...
**CODEX STATUS (2026-08-17).** Implemented the owner-map packet join and the only permitted
sub-line inheritance (for example, `11a`/`11b` inherit `11` only when no specific owner exists),
with `instruction_match` and `instruction_inherited` provenance on cell packets. Added the
deterministic two-sided report builder and checked in
`plans/m20_s116_instruction_reconciliation.yaml`, regenerated by
`pilot/reconcile_instructions.py`. The report covers line-anchored cells, topic booklets
(`schedule_1a` 0/48 and `schedule_b` 0/8 matched), and the 46 out-of-scope dependents cells.

RAN (PowerShell): `$env:PYTEST_DEBUG_TEMPROOT = 'C:\\Users\\devbox\\projects\\tax_graph\\.test_tmp2'` then `.venv\\Scripts\\python.exe -m pytest tests\\test_m20_s116.py -q` -> 4 passed.
RAN (PowerShell): `$env:PYTEST_DEBUG_TEMPROOT = 'C:\\Users\\devbox\\projects\\tax_graph\\.test_tmp2'` then `.venv\\Scripts\\python.exe -m pytest tests\\test_m20_s116.py tests\\test_instruction_sections_m20.py tests\\test_outline_span_resolution_m20.py tests\\test_derive_cells_m20.py -q` -> 96 passed.
RAN (PowerShell): `$env:PYTEST_DEBUG_TEMPROOT = 'C:\\Users\\devbox\\projects\\tax_graph\\.test_tmp2'` then `.venv\\Scripts\\python.exe -m pytest tests\\test_form_completeness_m20.py tests\\test_workbench_m15.py tests\\test_run_summary_m20.py -q` -> 10 passed.
