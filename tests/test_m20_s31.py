...
def test_all_prompt_templates_render_with_representative_values() -> None:
    ...
    values = {
        ...
        "document_inventory": "- form_1040_2025: Form 1040",
    ...
    assert 'For a sibling line on this same form, use only {"line": "7", "column": null}' in cell_text
    assert 'Use {"form": "form_XXXX_2025", "line": "7", "column": null} only for a line on another form.' in cell_text
    assert 'For a table value, keep the printed line and column separate' in cell_text
    ...
    assert "Instructions booklets are evidence only, never graph operands." in cell_text
