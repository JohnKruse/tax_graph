...
def test_1040_groups_repeated_titles_and_refuses_step_blocks() -> None:
    ...
    assert len({item.target.document_id for item in result.worksheets}) == 14
    ...
    assert sum(item.kind == "table_merged" for item in result.inventory) == 11
    title_findings = [item for item in result.findings if item.kind == "worksheet_title_missing"]
    assert len(title_findings) == 2
    assert any("tables 63" in item.message for item in title_findings)
    assert any("tables 98,99" in item.message for item in title_findings)


def test_same_title_tables_starting_at_line_one_are_refused() -> None:
    source = """
    ...
    finding = next(item for item in result.findings if item.kind == "ambiguous_same_title_merge")
    assert "table_id=1" in finding.evidence
    assert "table_id=2" in finding.evidence
