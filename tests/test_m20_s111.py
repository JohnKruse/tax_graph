"""M20-S111 guards for model-owned line classification and election projection."""
...
def test_formula_union_has_required_nullable_branch_fields() -> None:
    ...
    assert schema["properties"]["operation"]["type"] == ["string", "null"]
...
def test_non_computation_union_branches_validate_from_supplied_evidence(kind: str) -> None:
    plan = _union_base(kind)
    if kind == "filer_entry":
        pass
    elif kind == "information_return":
        plan.update({"form": "W-2", "box": "1"})
    else:
        plan["reason"] = "The supplied packet does not state the treatment."
...
def test_election_requires_escalation_and_supplied_citations() -> None:
    ...
    plan["options"] = [
        ...
        {
            "label": "Escalate",
            "downstream_effect": "Pause for a human decision.",
            "citation_refs": ["span_form"],
...
def test_formula_prompt_requires_grounding_and_exposes_cues_as_examples() -> None:
...
def test_model_election_is_projected_without_copy_rule() -> None:
    ...
    assert {item.data["citation_id"] for item in batch.items("citations")} == {
        "cite_span_form",
        "cite_span_instruction",
    }
    decision = batch.items("decisions")[0].data
    assert decision["question"] == "Do you want a refund or estimated tax credit?"
    assert {option["option_type"] for option in decision["options"]} == {"choice", "escalate"}
...
def test_model_node_set_is_wider_than_legacy_cue_set() -> None:
    ...
    assert [node.outline_id for node in _formula_outline_nodes(nodes)] == ["line_2"]
    assert [node.outline_id for node in _model_formula_outline_nodes(nodes)] == ["line_1", "line_2"]
