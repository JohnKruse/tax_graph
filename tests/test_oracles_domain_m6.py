...
def test_generated_scenarios_stay_inside_profile_bounds():
    ...
    assert all(scenario.extra_tax_graph_facts["schedule_d_2025_line_6_st_carryover"] <= 0 for scenario in scenarios)
    assert all(scenario.extra_ots_inputs["D6"] >= 0 for scenario in scenarios)
    assert all(scenario.extra_tax_graph_facts["schedule_d_2025_line_6_st_carryover"] == -scenario.extra_ots_inputs["D6"] for scenario in scenarios)
    assert all(scenario.extra_tax_graph_facts["schedule_d_2025_line_14_lt_carryover"] <= 0 for scenario in scenarios)
    assert all(scenario.extra_tax_graph_facts["schedule_d_2025_line_14_lt_carryover"] == -scenario.extra_ots_inputs["D14"] for scenario in scenarios)
    assert all(scenario.extra_tax_graph_facts["schedule_d_2025_line_18"] == scenario.extra_ots_inputs["Collectibles"] for scenario in scenarios)
    assert all(scenario.extra_tax_graph_facts["schedule_d_2025_line_19"] == scenario.extra_ots_inputs["D19"] for scenario in scenarios)
